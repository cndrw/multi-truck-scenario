...
class ImageToCppGridConverter:
    ...
    def get_color_of_pixel(self, rgb_value):
        """
        Maps an RGB color to a corresponding enum value based on specified ranges.
        """
    ...
    def convert_to_cpp_grid_data(self):
        """
        Convert pixel data to a linear grid data list.
        """
        static_map = []
...
def generate_rviz_static_map(image_path):
    """
    Generates an RViz-compatible static_map from an image.
    
    :param image_path: Path to the input image.
    :return: A dictionary with the RViz-compatible flattened grid and dimensions.
    """
