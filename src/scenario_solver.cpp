...
{
    int top_right = solve_S1_vorfahrt();
    
    const auto v = std::find_if(m_vehicles.begin(), m_vehicles.end(), [top_right](const auto& v){
        return v->vin == top_right;
    });

    float distance = std::numeric_limits<float>::max();
    const int winner_vin = INVALID_VIN;
}
...
int ScenarioSolver::solve_S1_vorfahrt()
{
